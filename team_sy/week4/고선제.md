...
- prompt() : 문자열을 사용자로부터 입력받을 수 있는 메소드.
- \*\*: 거듭제곱을 의미한다.
- Infinity : 무한을 나타낸다. type은 number(숫자)이다.

ex) 2/0은 Infinity이다.

(1) 연산자 우선순위 <br> -우리가 배운 수학에서의 연산 우선순위와 같다.
...
- 변수를 선언하지 않고도 기본적으로 제공해주는 도구들이 있다.
...
- 변수를 수정하고 싶을 때 처음 대입해준 값을 바꾸려면 변수를 선언하고 넣고싶은 값을 넣으면 된다.
...
- let이랑 기본적으로 같다.
- let이랑 다른 점은 같은 변수명으로 여러 번 선언할 수 있다는 점이다.
- 최근에 들어 잘 쓰지 않는다.
...
: 사용자에게 확인 or 취소를 입력 값으로 받는다.

: 확인을 누르면 true, 취소를 누르면 false를 반환한다.

- document.querySelector(’태그 이름’);

: argument에 들어간 태그를 선택한다.

: 태그가 여러 개 일 때:document.querySelectorAll(’태그 이름’);
