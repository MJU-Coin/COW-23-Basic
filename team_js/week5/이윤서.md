...
: document.createElement, document.createTextNode로 만든 태그나 텍스트를 선택한 태그의 자식 택로 넣는다. appendChild로는 하나만 넣을 수 있지만, append는 여러개를 동시에 넣을 수도 있다. append로 텍스트를 추가할 떄는 document.createTextNode 를 사용하지않고 바로 문자열을 넣는 것도 가능하다.
